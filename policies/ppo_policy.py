...
class PPOPolicy(actor_policy.ActorPolicy):
  ...
  def apply_value_network(self, observations, step_types, value_state=None):
    ...
    if self._observation_normalizer:
      _observations = self._observation_normalizer.normalize(observations["observation"])
    return self._value_network(OrderedDict({"observation":
            _observations, "z":observations["z"]}), step_types, value_state)
  ...
  def _apply_actor_network(self, time_step, policy_state):
    if self._observation_normalizer:
      ...
      time_step = ts.TimeStep(time_step.step_type, time_step.reward,
                              time_step.discount, OrderedDict({"observation":
            observation, "z":time_step.observation["z"]}))
